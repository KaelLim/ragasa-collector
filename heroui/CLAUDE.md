...
**本次會話完整記錄**: `visit-records-module-v1.2.0-development-2025-10-11`

包含：
- 完整開發過程（49 commits）
- 技術整合細節（Whisper STT + Qwen3）
- 已知問題與解決方案
- 待完成工作清單
...
#### 修復內容
- 🐛 **訪視記錄確認頁照片顯示**：步驟 6 確認頁面新增照片預覽功能
- 🎨 **登入頁面清理**：隱藏開發環境提示卡片和註冊連結
- 🔧 **開發環境快取清理**：解決錄音快取問題

#### 功能改進
1. **照片預覽系統** (`app/visit-records/new/steps/ConfirmationStep.tsx`)
   - 新增訪視互動照片預覽區域
   - 新增其他佐證照片預覽區域
   - 響應式網格佈局（2x3 欄）
   - 顯示照片數量統計
   - 使用 `URL.createObjectURL()` 客戶端預覽

2. **登入頁面優化** (`app/login/page.tsx`)
   - 隱藏開發環境共用密碼提示（行 113-121）
   - 隱藏註冊連結（行 123-128）
   - 保留註解以便未來恢復

#### 文檔更新
- 📝 **OpenMemory 記憶索引**：新增 OCR 地址校正和 Qwen3 優化文檔
  - 記憶 ID: `ed235671-1283-4923-b17c-f72ed20fa7c8`
  - 包含 OCR 後處理函數說明
  - 包含 Qwen3 系統提示詞改進
...
#### 相關 Commits
- `待提交` - fix: 新增訪視記錄確認頁照片預覽
- `待提交` - fix: 隱藏登入頁面開發環境提示
- `待提交` - docs: 新增 OpenMemory 記憶索引
...
### Whisper STT + Qwen3 地址校正優化

**OpenMemory 記憶 ID**: `ed235671-1283-4923-b17c-f72ed20fa7c8`

#### 修復問題
- 🐛 **OCR 地址誤判**：「中山路七段」被誤認為「中山路上段」
- 🐛 **Qwen3 模板文字**：輸出 `[正文內容在此]` 佔位符而非實際內容

#### 解決方案

**OCR API 後處理** (`app/api/ocr/route.ts`):
- 新增 `correctAddressOCRErrors()` 函數
- 使用正規表達式自動修正常見 OCR 錯誤
- 「上段」→「七段」、「下段」→「九段」
- 在 OCR 結果返回前即時修正

**Whisper API 系統提示增強** (`app/api/whisper/route.ts`):
- 加入地址校正規則到 Qwen3 系統提示
- 明確指示禁止將「七段」改為「上段」
- 說明「七」發音類似「上」但必須保持數字形式
- 比對戶口名簿地址資料確保準確度

#### 技術重點

**雙層校正機制**:
1. OCR 階段：視覺辨識錯誤的即時修正
2. Qwen3 階段：語意理解的規則約束
...
#### 相關檔案
- `app/api/ocr/route.ts` - OCR 後處理函數
- `app/api/whisper/route.ts` - Qwen3 系統提示

#### 測試建議
1. 重新上傳含有「七段」地址的戶口名簿
2. 檢查 Console 日誌確認修正訊息
3. 驗證錄音轉文字時地址正確性
4. 確認無模板佔位符文字出現
